...
export default function ({ store, app: { $axios }, redirect }) {
    ...
    $axios.onRequest((config) => {
      ...
      if (store.state.auth.access_token) {
        ...
        //$axios.setHeader("Authorization", 'Bearer ' + store.state.auth.access_token)
        console.log(`Set the header to ${'Bearer ' + store.state.auth.access_token}`)
      }
    ...
    $axios.onError((error) => {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        if ((statusCode === 401 || statusCode === 422) && !isIgnored) {
          ...
          // When access tokens expire or become invalid but the application still needs to access 
          // a protected resource, the application faces the problem of getting a new access token 
          // without forcing the user to once again grant permission. To solve this problem, OAuth 
          // 2.0 introduced an artifact called a refresh token. A refresh token allows an application 
          // to obtain a new access token without prompting the user.
          //  — “Understanding Refresh Tokens” (https://auth0.com/learn/refresh-tokens/) via auth0
